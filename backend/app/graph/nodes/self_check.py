...
async def self_check(state: RAGState) -> dict:
    ...
    # per_doc.relevant is no longer surfaced in the progress trace. Citation
    # checkmarks (✓) shown in the UI must reflect the answer-LLM's actual
    # `[N]` citations, which we only know after generate streams — chat.py
    # emits a post-generate progress chunk listing the cited docs. self_check
    # keeps just the overall sufficient verdict, which gates the retry loop.
    update: dict = {
